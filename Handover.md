...
|DRL based HO Management for mm Wave Communication|Deep Q-Learning, Deep Neural Network (DNN), rate based HO (RBH)|Optimal BS Selection based on DRL, one hot-code to describe BS, received SNR of all BSs is the important parameter for BS Selection, compare RBH with DQN| recevied SNR to bulid throughput as reward|Access-beam reference signal received power(RSRP) from serving and neighbor cells|choose target BS to HO|  
