...
|Optimization of HO problem using Q-Learning for LTE network|Q-Learning, LTE Hard HO, RSS based TTT Window, Integrator HO, LTE Hard HO with average RSRP Constraint|use Q-learning to find the optimal parameter of HO, such as HOM, TTT, the number of HO and throughput| The reward function is related to the number of HO, system throughput and delay. This work compare the performance of 4 standard LTE HO algorithm using optimal Parameter(HOM and TTT)|
|Multi-Agent DRL for distributed HO Management in dense mm-wave Networks|Multi-Agent DRL, Deep recurrent Q network(DQRN)|each UE is modeled as an independent agent that learns in a distributed way its handover strategy with the goal of optimizing the network throughput, Consider more parameter like RSS, ACK, data rate, network sum-rate|This work build two reward functions taking into account the collision events(fully cooperative RL HO and self interest RL HO), compare the performance(HO rate and throughput)of 2 reward function and RSS|  
|Learning-based Load Balancing HO in Mobile mm-Wave Networks|deep deterministic policy gradient(DDPG)|select a backup BS and allocates the resource to maximize the sum rate of all the users while ensuring a target rate threshold and preventing excessive handovers, use DDPG to optimize HO|DDPG method is applied to the ofﬂine training phase to estimate the optimal policy. Then, the obtained policy is used to the online phase.  The goal is to minimize rate requirement and the number of handovers|  
|A DQN-based HO Management for SDN-Enabled Ultra-Dense Networks(UDN)|DQN, UDN, software defined network(SDN)| use SDN to build UDN structure, use RL framework to build control plane of SDN|compare the HO rate and Average throughput of traditional , UCB-based HO and DQN based HO, the reward function is reated to throughput, the number of HO and HO failure|
